...
#include "../proc_comm.h"
//modified kkh8318	1121
//#include "lge_proc_comm.h"
#include <linux/platform_device.h>
//modified kkh8318	1121

//modified kkh8318	1121
#define CUSTOMER_CMD2_QEM_ENABLED_NV_MODE 	0x0012
#define CUSTOMER_CMD2_SET_FRST_STATUS 0x0015
#define CUSTOMER_CMD2_GET_FRST_STATUS 0x0016
//modified kkh8318	1121
...
static int lge_ftm_proc_comm(int cmd, int *cmd_result)
{
	...
	ret = msm_proc_comm(PCOM_CUSTOMER_CMD2, cmd_result, &cmd);
	if (ret < 0) {
		pr_err("%s: msm_proc_comm(PCOM_CUSTOMER_CMD2) failed(%d)\n",
			   __func__, ret);
		return ret;
	}
...
static int lge_ftm_get_qem(void)
{
	int qem = 0;
	//modified kkh8318	1121
	if(lge_ftm_proc_comm(CUSTOMER_CMD2_QEM_ENABLED_NV_MODE, &qem) < 0)
		return 0;
//modified kkh8318	1121
	return qem;
}
...
{
	lge_ftm_proc_comm(CUSTOMER_CMD2_SET_FRST_STATUS, &status);
}

static int lge_ftm_get_frst_status(void)
{
	int status = 0;
	if(lge_ftm_proc_comm(CUSTOMER_CMD2_GET_FRST_STATUS, &status) < 0)
		return 0;
	return status;
}
