...
// LGE_CHANGE_S, [myunghwan.kim@lge.com], 2011-11-15
#ifdef CONFIG_LGE_USB_GADGET_DRIVER
	udc_cable = lge_get_cable_info();
	tmp = lge_get_pif_info();
	printk(KERN_INFO " *** lge_get_cable_info : %x, pif : %x\n", udc_cable, tmp);

	is_manual_testmode = (udc_cable & LGE_CABLE_TYPE_NV_MANUAL_TESTMODE) != 0 ? 1 : 0;
	udc_cable &= LGE_CABLE_TYPE_MASK;

	// LT Cable
	//if (tmp == 2)
	if(is_manual_testmode == 1 ||
		(udc_cable == LGE_CABLE_TYPE_56K || udc_cable == LGE_CABLE_TYPE_130K || udc_cable == LGE_CABLE_TYPE_910K))
	{
...
#endif
// LGE_CHANGE_E, [myunghwan.kim@lge.com], 2011-11-15
