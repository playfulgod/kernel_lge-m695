...
static irqreturn_t gpio_irq_handler(int irq, void *dev_id)
{
	...
	wake_lock_timeout(&switch_data->wake_lock, 2*HZ); // 2 second
...
static int lge_gpio_switch_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&switch_data->wake_lock, WAKE_LOCK_SUSPEND, "lge_gpio_switch");
...
err_switch_dev_register:
	kfree(switch_data);
	wake_lock_destroy(&switch_data->wake_lock);
