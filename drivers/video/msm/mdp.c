...
int mdp_write_kcal_reg(const char* buf)
{
	...
	//sscanf(buf, "%3d%3d%3d", &cal_R, &cal_G, &cal_B);
  /*cal_R = buf[0];
	cal_G = buf[1];
	cal_B = buf[2];//*/
...
/*
static char mdp_kcal=0;
static ssize_t mdp_kcal_show(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	
	return count;//snprintf(buf, PAGE_SIZE, "%c\n",&mdp_kcal );
}

static ssize_t mdp_kcal_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	sscanf(buf, "%c", &mdp_kcal);
	mdp_write_kcal_reg(buf);
	return count;
}

static DEVICE_ATTR(mdp_cal,  S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP, mdp_kcal_show, mdp_kcal_store);

//static DEVICE_ATTR(mdp_kcal,S_IWUGO,NULL,mdp_write_kcal);

static struct attribute *mdp_attrs[]={
	&dev_attr_mdp_cal.attr,
	NULL,
};
static struct attribute_group mdp_attr_group ={
	.attrs = mdp_attrs,
};
*/
#endif
